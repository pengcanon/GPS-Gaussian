...
def local_correlation_softmax(feature0, feature1, local_radius,
    ...
    #the following two lines replaces the original line above
    corr = corr.half()                  # stays FP16
    corr[~valid] = torch.finfo(corr.dtype).min  # use min value for softmax
...
def global_correlation_softmax_stereo(feature0, feature1,
    ...
    """ code for debugging memory usage"""
    """def memMB():
        return torch.cuda.memory_allocated() / 1024**2

    print("[0] before corr", memMB(), "MB")"""

    correlation = torch.matmul(feature0, feature1) / (c ** 0.5)  # [B, H, W, W]


    """print("corr shape", correlation.shape, "dtype", correlation.dtype)
    print("[1] after corr", memMB(), "MB")"""
    """end of code for debugging memory usage for correlation"""
    ...
    #correlation[~valid_mask] = -1e9 #correlation[~valid] = torch.finfo(correlation.dtype).min

    #the following two lines replaces the original line above
    correlation = correlation.half()                  # stays FP16
    correlation[~valid_mask] = torch.finfo(correlation.dtype).min

    prob = F.softmax(correlation, dim=-1)  # [B, H, W, W]

    """ code for debugging memory usage for softmax"""
    # -------------- right after soft-max ------------
    """print("[2] after softmax", memMB(), "MB")"""
    """end of code for debugging memory usage for softmax"""
